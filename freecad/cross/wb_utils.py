...
def git_init_submodules(
    ...
    def git_deinit_submodules():
        if not (MOD_PATH / '.git').exists():
            message('RobotCAD: skipping git submodule deinit (Mod directory is not a Git checkout).')
            return
    ...
    def git_update_submodules(update_from_remote_branch_param: str = ''):
        if not (MOD_PATH / '.git').exists():
            message(
                'RobotCAD: skipping git submodule update because the Mod directory '
                'is not a Git checkout (no `.git`). Use a Git clone to update '
                'submodules, or rely on bundled module files.',
            )
            return
        ...
        try:
            ...
            if stderr:
                fc.Console.PrintError('git submodule update failed:\n' + stderr + '\n')
            raise
        print('process:', p)
